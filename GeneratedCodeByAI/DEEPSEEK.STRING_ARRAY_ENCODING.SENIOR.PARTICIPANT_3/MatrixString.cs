...
namespace DEEPSEEK.STRING_ARRAY_ENCODING.SENIOR.PARTICIPANT_3
{
    public class MatrixString
    {
        ...
        {
            if (rows <= 0 || cols <= 0)
                throw new ArgumentException("Rows and cols must be positive integers.");
        ...
        public string RowToString(int index, string separator)
        {
            ...
            if (index < 0 || index >= m.GetLength(0))
                throw new MatrixException("Invalid row index.");

            string[] row = new string[m.GetLength(1)];
